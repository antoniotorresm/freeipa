...
def client_setup_dns(server, options):
    ...
    ipautil.copy_template_file(
        ...
        paths.UNBOUND_CONF,
        dict(
            TLS_CERT_BUNDLE_PATH=options.dns_over_tls_cert,
...
def install_check(options):
    ...
    if options.dns_over_tls:
        dnsutil.dns_policy = options.dns_policy
        dnsutil.dns_verify = options.dns_over_tls_cert
    ...
    # In case of DoT, IPA discovery will return DOT_SETUP_REQUIRED
    # before we talk to LDAP servers. Unbound needs to be configured
    # and discovery restarted.
    if options.dns_over_tls and ret == discovery.DOT_SETUP_REQUIRED:
        client_setup_dns(ds.kdc, options)
        dnsutil.dns_policy = None
        dnsutil.dns_verify = True
        dnsutil.reset_default_resolver()
        if isinstance(ds.kdc, list):
            kdcs = ds.kdc
        else:
            kdcs = [ds.kdc]
        ret = ds.search(
...
class ClientInstallInterface(hostname_.HostNameInstallInterface,
    ...
    * ipa-replica-install
    ...
    dns_over_tls_cert = knob(
        str, None,
        description="Certificate used to connect to DoT server"
    )
    ...
    def __init__(self, **kwargs):
        ...
        if self.dns_over_tls:
            if not self.dns_over_tls_cert:
                raise RuntimeError(
                    "--dns-over-tls cannot be used without "
                    "--dns-over-tls-cert"
                )
            if not self.dns_policy:
                raise RuntimeError(
                    "--dns-over-tls cannot be used without "
                    "--dns-policy"
                )
